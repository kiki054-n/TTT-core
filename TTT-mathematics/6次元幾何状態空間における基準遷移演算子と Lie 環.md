...
数学的議論の成果と定式化のプロセスを、再現性と厳密性を備えた技術ドキュメント（Markdown形式）として以下にまとめました。そのままコピー＆ペーストして `.md` ファイルとして保存・保管していただけます。
...
$$\mathbf{p}_1 = D_3 = \begin{pmatrix} 5 \\ 9 \\ 6 \\ 0.884286376006 \\ 1.369438406005 \\ 0.866025403784 \end{pmatrix}, \quad \mathbf{p}_2 = C_6 = \begin{pmatrix} 8 \\ 12 \\ 6 \\ 0.551285598433 \\ 1.570796326795 \\ 1.539600717839 \end{pmatrix}$$

$$\mathbf{p}_3 = O_8 = \begin{pmatrix} 6 \\ 12 \\ 8 \\ 1.359347637816 \\ 1.910633236249 \\ 1.333333333333 \end{pmatrix}, \quad \mathbf{p}_4 = D_5 = \begin{pmatrix} 7 \\ 15 \\ 10 \\ 1.663999068981 \\ 2.192429488252 \\ 1.585094193825 \end{pmatrix}$$
...
$$T_i = \arg\min_T \Vert{}T - I\Vert{}_F \quad \text{s.t.} \quad T_i \mathbf{p}_i = \mathbf{p}_{i+1}$$

より、各ステップの基準遷移演算子 $T_i \in \mathbb{R}^{6\times 6}$ は階数1（Rank-1）更新として解析的に一意決定される。

$$\mathbf{\Delta}_i = \mathbf{p}_{i+1} - \mathbf{p}_i \implies \mathbf{T_i = I + \frac{\mathbf{\Delta}_i \mathbf{p}_i^T}{\mathbf{p}_i^T \mathbf{p}_i}}$$

---

## 3. 合成演算子 $T$ のブロック構造と $T_2$ の双対性

合成遷移演算子 $T = T_3 T_2 T_1$ を $3 \times 3$ ブロック行列分解する：
...
### $T_2$（$C_6 \to O_8$）における Cube–Octahedron 双対性分解

完全双対置換行列 $S = \begin{pmatrix} 0 & 0 & 1 \\ 0 & 1 & 0 \\ 1 & 0 & 0 \end{pmatrix}$ を用いて $A_2 = S + \Delta A_2$ に分解すると、

$$\Delta A_2 = \begin{pmatrix}  0.93577952 & -0.09633073 & -1.04816536 \\ 0 & 0 & 0 \\ -0.93577952 & 0.09633073 & 1.04816536  \end{pmatrix}$$
...
合成演算子 $T = T_3 T_2 T_1$ のスペクトル（固有値集合）は以下の通りである。
...
各ステップの連続生成元 $K_i = T_i - I = \mathbf{u}_i \mathbf{v}_i^T$（ただし $\mathbf{u}_i = \mathbf{\Delta}_i, \, \mathbf{v}_i = \frac{\mathbf{p}_i}{\Vert{}\mathbf{p}_i\Vert{}^2}$）の非可換交換子：

$$[K_i, K_j] = (\mathbf{v}_i^T \mathbf{u}_j) \mathbf{u}_i \mathbf{v}_j^T - (\mathbf{v}_j^T \mathbf{u}_i) \mathbf{u}_j \mathbf{v}_i^T$$
...
共役結合行列 $G_{ij} = \mathbf{v}_j^T \mathbf{u}_i$ をスカラー部（膨張）とトレースレス部 $G_0 \in \mathfrak{sl}(3, \mathbb{R})$ に分解する：

$$G_0 = G - \frac{1}{3}\operatorname{Tr}(G) I_3 \quad (\operatorname{Tr}(G) = 0.51527609)$$
...
各区間 $t \in [i-1, i]$ における連続生成元 $K_i \in \mathfrak{gl}(3, \mathbb{R})$：

$$K_i = \ln(T_i) = \left( \frac{\ln(1 + \mathbf{v}_i^T \mathbf{u}_i)}{\mathbf{v}_i^T \mathbf{u}_i} \right) \mathbf{u}_i \mathbf{v}_i^T$$
...
全区間を滑らかに結ぶ自律生成元 $K_{\text{global}} = \ln(T_3 T_2 T_1) \in \mathbb{R}^{6\times 6}$ の数値行列：

$$K_{\text{global}} = \begin{pmatrix} 0.038972 & 0.093407 & 0.078564 & 0.014848 & 0.016553 & 0.006404 \\ 0.155070 & 0.290288 & 0.193527 & 0.030199 & 0.044958 & 0.029594 \\ 0.116098 & 0.196882 & 0.114963 & 0.015351 & 0.028406 & 0.023190 \\ 0.025476 & 0.039022 & 0.019445 & 0.001930 & 0.005174 & 0.005175 \\ 0.023713 & 0.040300 & 0.024118 & 0.003234 & 0.005846 & 0.004543 \\ 0.017507 & 0.034371 & 0.024611 & 0.004033 & 0.005504 & 0.003103 \end{pmatrix}$$

**大域解軌跡:**

$$\mathbf{p}(t) = \exp(t K_{\text{global}}) \mathbf{p}_1 \quad (t \in [0, 1])$$

* $t = 0.00 \implies \mathbf{p}(0) = \mathbf{p}_1 \ (D_3)$
* $t = 0.33 \implies \mathbf{p}(0.33) \approx \mathbf{p}_2 \ (C_6)$
* $t = 0.67 \implies \mathbf{p}(0.67) \approx \mathbf{p}_3 \ (O_8)$
* $t = 1.00 \implies \mathbf{p}(1) = \mathbf{p}_4 \ (D_5)$
