...
<!-- TTT-STATUS v1 -->
> **状態**: C（仮説・未検定） — 検定を通っていない
> **判定**: M1/M2 は再現できる（水準A）。ただし M3・M4（λ=1 の三重解と9次元閉合）は乱数4ベクトルでも再現される構成の産物、M5（rank-1 優位 99.64%）は乱数対照 77.9〜98.3% の中、M6（歪み/旋回比 2.256）は単位依存で不変量ではない。無次元化規約を先に定めれば修理可能。
