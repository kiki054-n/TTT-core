...
# ================================================================ 追記（同日 15時台）
# ---------------------------------------------------------------- A11
# 対称群 G を持つ次数 B の写像：分岐点 2B−2 個は G の軌道の和 → 最小の非自明な B
orbits = {'T': [4, 4, 6, 12], 'O': [6, 8, 12, 24], 'I': [12, 20, 30, 60]}
minB = {}
for g, orb in orbits.items():
    sums = {0}
    for _ in range(6):
        sums |= {s + o for s in sums for o in orb}
    minB[g] = min((s + 2) // 2 for s in sums if s > 0)
assert minB == {'T': 3, 'O': 4, 'I': 7}
ok("A11", f"非自明な対称写像の最小次数 T→3, O→4, I→7（スカーミオン B=3,4,7 を再現）")

# ---------------------------------------------------------------- A12
# 磁束 n のゼロモード（スピン j=(n−1)/2）が正四面体（T / 2T）で既約か
def normT(j):
    if float(j).is_integer():
        cl = [(0, 1), (2 * math.pi / 3, 8), (math.pi, 3)]; order = 12
    else:
        cl = [(0, 1), (2 * math.pi, 1), (2 * math.pi / 3, 4), (2 * math.pi / 3 + 2 * math.pi, 4),
              (4 * math.pi / 3, 4), (4 * math.pi / 3 + 2 * math.pi, 4), (math.pi, 3), (3 * math.pi, 3)]; order = 24
    def c(t):
        s = math.sin(t / 2)
        if abs(s) < 1e-12:
            return (2 * j + 1) * (1 if abs(math.cos(t / 2) - 1) < 1e-12 else (-1) ** int(round(2 * j)))
        return math.sin((2 * j + 1) * t / 2) / s
    return round(sum(k * c(t) ** 2 for t, k in cl) / order, 6)
irr_n = [n for n in range(1, 9) if normT((n - 1) / 2) == 1]
assert irr_n == [1, 2, 3]
ok("A12", "磁束 n のゼロモードが正四面体の下で既約なのは n = 1, 2, 3 のみ（n=3 は A4 の三重項）")

# ---------------------------------------------------------------- A13/A14
def ico(swap):
    V = []
    for a, b in itertools.product([1, -1], repeat=2):
        pnt = (0, a, b * phi) if not swap else (0, a * phi, b)
        V += [pnt, (pnt[1], pnt[2], pnt[0]), (pnt[2], pnt[0], pnt[1])]
    return np.array(V, float)
def rotgroup(V):
    Gs = []
    for tri in itertools.permutations(range(12), 3):
        try:
            M = np.linalg.solve(V[[0, 1, 4]], V[list(tri)]).T
        except np.linalg.LinAlgError:
            continue
        if not np.allclose(M @ M.T, np.eye(3), atol=1e-9) or np.linalg.det(M) < 0:
            continue
        if all(np.isclose(np.linalg.norm((M @ V.T).T - v, axis=1), 0, atol=1e-9).any() for v in V) \
                and not any(np.allclose(M, g) for g in Gs):
            Gs.append(M)
    return Gs
VA, VB = ico(False), ico(True)
GA, GB = rotgroup(VA), rotgroup(VB)
com = [g for g in GA if any(np.allclose(g, h) for h in GB)]
angc = Counter(round(math.degrees(math.acos(np.clip((np.trace(g) - 1) / 2, -1, 1)))) for g in com)
assert len(GA) == len(GB) == 60 and len(com) == 12 and angc == Counter({0: 1, 120: 8, 180: 3})
ok("A13", "90°回転の関係にある二つの正二十面体の共通回転対称 = 位数12の正四面体群 T")

def faces(V):
    D = []
    for i, j, k in itertools.combinations(range(12), 3):
        if all(np.isclose(np.linalg.norm(x - y), 2) for x, y in [(V[i], V[j]), (V[j], V[k]), (V[i], V[k])]):
            nrm = V[i] + V[j] + V[k]; D.append(nrm / np.linalg.norm(nrm))
    return np.array(D)
FA, FB = faces(VA), faces(VB)
shared = np.array([f for f in FA if np.isclose(np.linalg.norm(FB - f, axis=1), 0, atol=1e-9).any()])
cube = np.array(list(itertools.product([1, -1], repeat=3))) / math.sqrt(3)
assert len(shared) == 8 and all(np.isclose(np.linalg.norm(cube - s, axis=1), 0).any() for s in shared)
H3 = holes(3)
assert all(np.isclose(np.linalg.norm(cube - h, axis=1), 0, atol=1e-7).any() for h in H3)
assert len({int(np.sign(np.prod(h))) for h in H3}) == 1
ok("A14", "共有する面の向き = 8 = 立方体頂点 = XYZ=+1 と XYZ=−1 の二つの正四面体；次数3写像の穴はその片方（どちらかは向きの規約）")

# ---------------------------------------------------------------- A15
def ax_sets(V):
    nz = lambda v: v / np.linalg.norm(v)
    E = [nz(V[i] + V[j]) for i, j in itertools.combinations(range(12), 2) if np.isclose(np.linalg.norm(V[i] - V[j]), 2)]
    L = [nz(v) for v in V] + list(faces(V)) + E
    out = []
    for d in L:
        if not any(abs(abs(d @ e) - 1) < 1e-9 for e in out):
            out.append(d)
    return out
XA, XB = ax_sets(VA), ax_sets(VB)
assert len(XA) == 31
langle = lambda u, v: round(math.degrees(math.acos(min(1, abs(u @ v)))), 4)
cand = {langle(u, v) for u in XA for v in XB} | {langle(u, v) for u in XA for v in XA}
cand.discard(0.0)
small = sorted(a for a in cand if a < 45)
lo3, hi3 = math.degrees(math.asin(math.sqrt(0.0201))), math.degrees(math.asin(math.sqrt(0.0243)))
assert len(cand) == 44 and len(small) == 16 and not [a for a in cand if lo3 <= a <= hi3] and min(small) == 10.8123
der = {round(x / 2, 4) for x in small} | {round(x - y, 4) for x in small for y in small if x > y}
in1 = sorted(a for a in der if abs(a - 8.54) <= 0.13)
assert len(der) == 131 and in1 == [8.4775, 8.5651]
pois = 1 - math.exp(-len(der) * 0.26 / 45) * (1 + len(der) * 0.26 / 45)
ok("A15", f"軸間の角度 44 種（45°未満 16 種、最小 10.81°）に θ13 の3σ窓 [{lo3:.2f},{hi3:.2f}]° 入りなし；"
          f"半角・差まで許すと 131 種中 1σ 窓に {in1}（偶然で2個以上入る確率 ≈ {pois:.2f}）")
