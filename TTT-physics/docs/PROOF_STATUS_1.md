...
| 3 | **R2 downgraded from refuted to open** (→ O3) | v1.0 rejected $648=501+147$ "within the stated TTT counting rules," while U3 simultaneously declared those counting rules undetermined. A rejection resting on an unresolved premise is not a rejection. |
| 4 | R3's contested part relocated to O3 | $147 \neq 37\times4$ is unconditionally false and stays refuted. The live question — whether the overflow quantity is 147 or 148 — is a counting question and belongs with O3. |
...
| 12 | Numerical targets stated at their measured precision | e.g. $m_p/m_e = 1836.152673$, not 1836. |
...
| **P2** | $\pi=-1$ / spin | **D** | **C** | Hamilton ($ijk=-1$); $SU(2)$ double cover |
...
| **P7** | Pentagonal bipyramid = 7 | **D** | **C** | Vertex counting |
| **P8** | Centre quantity / boundary | **A + E** | **C** | Hydrostatic equilibrium |

**Reading of this table.** Exactly one entry (P6) reaches theorem level for the statement TTT actually wants to make, and that theorem is borrowed. P3, P4, P5 and P8 correctly use established physics as premises, but the TTT generalisation built on each is at hypothesis level. P1, P2 and P7 are definitions.
...
| **R3** | $147=37\times4$ | **F** | $37\times4=148$. (Whether the overflow is 147 or 148 → **O3**) |
...
| ~~R2~~ | $648=501+147$ | *moved* | Was F in v1.0; its rejection depended on the unresolved count → **O3** |

## 2.3 Open problems (O)

| ID | Subject | Level | Absorbs | Core issue |
|---|---|:---:|---|---|
| **O1** | Unit-conversion rule | **O** | U1, U4, U5, U6 | No rule maps TTT quantities to physical units |
| **O2** | Boundary component count | **B** | U2 | $\partial\partial=0$ gives 4, not 5; needs formalising |
| **O3** | Structural count 124 / 125 / 128 | **O** | U3, R2, part of R3 | Which count is fundamental is undetermined |
...
v1.0 graded this M3. The algebra is valid, but both premises are TTT stipulations chosen so that this result follows. One line of arithmetic from two chosen premises is a definition restated, not a theorem.

What *is* at level A is external and must be attributed: Hamilton's $ijk=-1$, and $U(2\pi)=-\mathbb{1}$ in $SU(2)$. If $X,Y,Z$ are read as half-turns about three axes, TTT's $\pi$ is structurally the same object as the non-trivial element of the double cover. That identification is a hypothesis about TTT, not a consequence of it.
...
v1.0 graded the count M3 and called it "mathematically exact." It is exact, and it is a count. Grading arithmetic as a proven theorem inflates the mathematical content of the theory; that is precisely the failure mode this document exists to prevent.

The non-trivial part is P6's restriction $n\le5$, which is what makes the base pentagonal rather than arbitrary. The credit belongs there, not here.
...
The arithmetic is settled. The live question is a different one: **is the overflow quantity 147 or 148?** That depends on the counting convention, which is unresolved. It is tracked under **O3**, not here.

This separation matters. v1.0's ledger downgraded R2 and R3 to "contested," but the contested content of R3 is not its arithmetic — it is which quantity the arithmetic should be applied to. Keeping the false identity marked F while moving the real question to O3 preserves both the rejection and the dispute.
...
*(absorbs U1 "energy scale of 125.25", U4 "density = 1", U5 "origin of 0.25", U6 "scale mechanism")*
...
Solving any one in isolation does not help; solving the conversion rule solves all four. Consequently **O1 is the single highest priority in the project**, and §22's priority list collapses accordingly.
...
*(absorbs U3, the contested part of R2, the contested part of R3, and the 37/38 parity issue)*
...
**From R2.** v1.0 rejected $648=501+147$ on the grounds that TTT counting gives $125\times4=500$, hence $648-500=148$. But that computation assumes 1 tetrahedron = 1 cell — the very convention this section says is undetermined, and one that is incompatible with premise 1 above. **A rejection cannot rest on an unresolved premise.** R2 is therefore not refuted; it is undecided, and it will be decided by whichever counting convention O3 settles on.

Note that $501+147=648$ is arithmetically correct. Whether the structurally meaningful split is $500+148$ or $501+147$ is a question about what the quantities mean, not about arithmetic.

**From R3.** Whether the overflow is 147 or 148.
...
**Consequence.** Under three different readings used within a single day (125 cells × 4 = 500 vs 648 → overflow 148; 125 = count of $O$; 125 = unit with 31 tetrahedra fitting vs 162 → overflow 131), **37 does not emerge from any of them.** Until the convention is fixed, no overflow number carries meaning.
...
$\pi=-1$ under its stated premises; $\kappa=1/r$; the crystallographic restriction; $5+2=7$; the arithmetic refutations R1, R3, R4.

Evaluable by ordinary mathematics. **Note that three of these five are definitions or arithmetic, and the one real theorem is borrowed.**
...
Establish the unit-conversion rule. Then predict **one independent quantity other than $m_H$** and compare. Nothing else in the theory can be tested until this exists.
...
Derive 124 or 128 from an axiom and show the alternative contradicts. This also settles R2, the 147/148 question, and the 37/38 parity issue in one stroke.
